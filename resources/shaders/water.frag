#version 330 core
// Water scene fullscreen shader (Shadertoy-style adaptation)
// Source adapted from user-provided code (afl_ext 2017-2024, MIT)
// Inputs:
//  - gl_FragCoord.xy: pixel coordinates
//  - uniforms: iResolution (xy = viewport size), iTime (seconds), iMouse (xy = cursor in pixels)
//
// Outputs:
out vec4 fragColor;
in vec2 v_uv; // from post.vert (unused; we use gl_FragCoord)
...
// ============== Original shader code begins (lightly adapted) ==============
...
void mainImage(out vec4 fragColor_, in vec2 fragCoord) {
  ...
  if(ray.y >= 0.0) {
    ...
    fragColor_ = vec4(aces_tonemap(C * 2.0),1.0);   
    return;
  }
